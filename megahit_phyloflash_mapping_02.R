...
# Filter for pident between 95 and 100 as before
data_100 <- data %>% filter(between(pident, 99, 100))

# Define length_category based on 'length' column and the target size 1000 ± 10 bp
data_100 <- data_100 %>%
  mutate(length_category = case_when(
    length < (1500 - 10) ~ "less than 1500 bp",
    length >= (1500 - 10) & length <= (1500 + 10) ~ "1500 bp (±10)",
    length > (1500 + 10) ~ "greater than 1500 bp"
  ))
...
all_categories <- c("less than 1500 bp", "1500 bp (±10)", "greater than 1500 bp")
...
ggplot(filter(complete_df, megahit_contig_count > 0), aes(x = megahit_contig_count, y = Sample_ID, fill = length_category)) +
  ...
  labs(
    title = "MEGAHIT-Barrnap 16S SSU rRNA contigs with ≥99% identity to Phyloflash contigs",
    y = "Coral Samples (sorted by total sequence count, highest top)",
    x = "Number of MEGA-HIT contigs",
    fill = "Length of Alignment"
  ) +
  scale_x_continuous(
...
# Join total_count info into summary_df and sort descending by total
summary_df_sorted <- summary_df %>%
  left_join(sample_totals, by = "Sample_ID") %>%
  arrange(desc(total_count)) %>%
  select(-total_count)  # If you want to remove extra column

cat("\nSummary Table (sorted by total sequences, highest to lowest):\n")
print(summary_df_sorted, n = Inf)
