...
# Summarize counts of unique OTUs by Sample and taxonomy levels
summary_df <- data %>%
  group_by(Sample, superkingdom, supergroup) %>%
  summarise(contig_count = n_distinct(OTU), .groups = "drop") %>%
  # Combine taxonomy levels for fill assignment
  mutate(taxonomy_full = paste(superkingdom, supergroup, sep = " | "))
...
summary_df <- data %>%
  group_by(Sample, superkingdom, supergroup) %>%
  summarise(
    contig_count = n_distinct(OTU),
    coverage = round(mean(read_cov, na.rm = TRUE), 1),  # or sum, median, etc. depending on your desired metric
    .groups = "drop"
  ) %>%
  mutate(taxonomy_full = paste(superkingdom, supergroup, sep = " | "))
...
summary_df$Sample <- factor(summary_df$Sample, levels = sample_totals$Sample)
...
# Plot
ggplot(summary_df, aes(x = contig_count, y = Sample, fill = taxonomy_full)) +
  geom_bar(stat = "identity", position = "stack", color = "black") + 
  scale_fill_manual(
    values = custom_colors,
    breaks = ordered_taxa,   # only affects legend and stacking order in plot
    name = "Taxonomy (Superkingdom | Supergroup)"
  ) +
  scale_x_continuous(
    breaks = seq(0, max(sample_totals$total_contigs), by = 1),
  ...
  geom_text(aes(label = coverage),
            ...
            color = "white") +
  labs(
    title = "Moorea Corals: Phyloflash 16S SSU rRNA Contigs with average read_cov per bacterial taxa",
    x = "Number of Contigs",
...
samples_without_cnidaria <- setdiff(levels(summary_df$Sample), samples_with_cnidaria)

cat("\nSamples with NO Cnidaria contigs:\n")
print(samples_without_cnidaria)

# For samples without Cnidaria, list all their contigs
# Assuming 'data' dataframe contains OTU, Sample, and taxonomy info for each contig
if ("OTU" %in% colnames(data)) {
  no_cnidaria_contigs <- data %>%
    filter(Sample %in% samples_without_cnidaria) %>%
    filter(!grepl("Cnidaria", supergroup)) %>%
    select(Sample, OTU) %>%
    distinct()
  ...
  cat("\nNo OTU or contig-level data available to list contigs without Cnidaria.\n")
}
